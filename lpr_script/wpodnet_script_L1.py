...
def run(weights='yolov5s.pt',  # model.pt path(s)
    ...
    global YOLO_Image  # Declare YOLO_Image as a global variable
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        # First Image (After YOLO) saved here
        if len(det):
           ...
           for idx, (*xyxy, conf, cls) in enumerate(det):
               ...
               # Read the JPG image as bytes
               with open(r'vehicle_images\test\images\1_vehicle_0.jpg', 'rb') as image_file:
                    image_bytes = image_file.read()
                   
               # Convert the bytes to a blob
               YOLO_Image = base64.b64encode(image_bytes).decode()   
...
@sio.event
async def connect(environ=None):
...
# Function to send the blob image
async def send_blob_image(image_blob):
    await sio.emit('data', {'type': 'image', 'content': image_blob})
...
async def main():
    try:
        ...
        # Send the blob image
        await send_blob_image(YOLO_Image)
        
        print('YOLO_Image is on server')
