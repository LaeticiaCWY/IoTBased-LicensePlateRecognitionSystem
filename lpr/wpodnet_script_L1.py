...
async def send_results_to_backend(result_type, result_data):
    async with websockets.connect("ws://localhost:5000/ws") as websocket:
        message = {"type": result_type, "data": result_data}
...
vehicle_detection_result = test_image
...
plt.ion()
plt.show(block=True)
license_plate_detection_result = LpImg[0]
...
for idx in range(len(result)):
    ...
    for line in res:
        print(line)
character_recognition_result = line



asyncio.get_event_loop().run_until_complete(send_results_to_backend("vehicle_detection", vehicle_detection_result))
asyncio.get_event_loop().run_until_complete(send_results_to_backend("license_plate_detection", license_plate_detection_result))
asyncio.get_event_loop().run_until_complete(send_results_to_backend("character_recognition", character_recognition_result))
