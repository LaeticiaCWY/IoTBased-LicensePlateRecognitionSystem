...
const ImageDisplay = () => {
  ...
  useEffect(() => {
    // Function to get the initial image URL from Firebase Storage with timestamp
    const getImageUrlWithTimestamp = async (imagePath) => {
      try {
        const imageRef = storage.ref(imagePath); // Replace with your image path
        const timestamp = Date.now(); // Get the current timestamp
        const url = await imageRef.getDownloadURL();
        return `${url}?timestamp=${timestamp}`; // Append timestamp as a query parameter
      } catch (error) {
    ...
    // Function to set image URLs with timestamp
    const setImageUrlsWithTimestamp = async () => {
      const imageUrl1 = await getImageUrlWithTimestamp("lpr/YOLO.jpg");
      setImageUrl1(imageUrl1);

      const imageUrl2 = await getImageUrlWithTimestamp("lpr/WPOD-NET.jpg");
      setImageUrl2(imageUrl2);
    };

    setImageUrlsWithTimestamp(); // Call the function to fetch and set image URLs with timestamps
